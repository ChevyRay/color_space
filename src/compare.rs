use crate::{FromColor, Lab, Lch, ToRgb};
...
impl<T: ToRgb, U: ToRgb> CompareEuclidean<U> for T {
    fn compare_euclidean(&self, color: &U) -> f64 {
        let a = self.to_rgb();
        let b = color.to_rgb();
        ((a.r - b.r) * (a.r - b.r) + (a.g - b.g) * (a.g - b.g) + (a.b - b.b) * (a.b - b.b)).sqrt()
    }
}
    ...
    Lab: FromColor<U>,
{
    fn compare_cie2000(&self, color: &U) -> f64 {
        ...
        let t = 1.0 - 0.17 * (mean_h - deg_to_rad(30.0)).cos()
            + 0.24 * (2.0 * mean_h).cos()
    ...
    Lch: FromColor<U>,
{
    fn compare_cmc(&self, color: &U) -> f64 {
        ...
        (sqr(delta_l / (2.0 * sl)) + sqr(delta_c / (1.0 * sc)) + sqr(delta_h / sh)).sqrt()
    }
}
